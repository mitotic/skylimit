...
import { updateSeenNotifications } from '../api/notifications'
import { getProfiles } from '../api/profile'
import { loadNotificationsWithPosts, NotificationWithPost } from '../api/notificationLoader'
import { getNotificationsCache, setNotificationsCache, invalidateNotificationsCache } from '../prefetch/prefetchCache'
...
export default function NotificationsPage() {
  ...
  // Helper to batch-check follow statuses and mark notifications as seen
  const runPostLoadTasks = useCallback(async (
    notifs: NotificationWithPost[],
    ...
    // Mark notifications as seen (skip in read-only mode, only on initial load)
    if (isInitialLoad) {
      if (isReadOnlyMode()) {
  ...
  const loadNotifications = useCallback(async (cursor?: string) => {
    ...
    try {
      ...
      // Check prefetch cache for initial load (not pagination)
      if (!cursor) {
        const cached = getNotificationsCache()
        if (cached) {
          log.debug('Notifications', 'Using prefetched notifications')
          setNotifications(cached.notifications)
          setCursor(cached.cursor)
          setIsLoading(false)
          // Still mark as seen and check follow statuses — user is actually viewing
          await runPostLoadTasks(cached.notifications, true)
          return
        }
      }

      // On-demand fetch via extracted loader
      const result = await loadNotificationsWithPosts(agent, {
        cursor,
        limit: 25,
        onRateLimit: (info) => {
          setRateLimitStatus({
      ...
      if (cursor) {
        ...
        setNotificationsCache(result)
      }

      // Batch fetch follow statuses and mark as seen in parallel (non-blocking)
      await runPostLoadTasks(result.notifications, !cursor)
  ...
  }, [agent, session, setRateLimitStatus, runPostLoadTasks])
  ...
  const handleLoadMore = () => {
    if (cursor && !isLoadingMore) {
      ...
      invalidateNotificationsCache() // multi-page state can't be cached
